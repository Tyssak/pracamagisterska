...
class PreProcessor:
    ...
    def run_face_detection(self, selected_option, selected_filter_option, file_path, frame_substraction = False):
        ...
        while True:
            ...
            if just_face is not None and frame_face is not None:
                ...
                #output_frame = apply_filter(just_face, my_filter, selected_filter_option=self.filter_option)

                iteration = self.show_frame(output_frame, iteration)
    ...
    def extract_face(self, frame):
        ...
        if face is not None:
            ...
            frame_face = crop_around_face(frame, face, scale=1.4)
    ...
    def apply_mask(self, frame, left_eye, right_eye, nose, center, rotation_matrix):
        ...
        # cv2.circle(frame, (new_left_eye_pos[0], new_left_eye_pos[1]), 4, (255, 0, 0), -1)
        # cv2.circle(frame, (new_right_eye_pos[0], new_right_eye_pos[1]), 4, (255, 0, 0), -1)
        # cv2.circle(frame, (new_nose_pos[0], new_nose_pos[1]), 4, (0, 0, 255), -1)
        # cv2.circle(frame, (new_center[0], new_center[1]), 4, (0, 255, 0), -1)

        width = min(int(scale_x * distance_between_eyes), frame.shape[0] // 2)
        height = min(int(scale_y * distance_to_nose), frame.shape[1] // 2)
    ...
    def change_coordinates(self, just_face, frame_face, left_eye, right_eye, nose):
        cord_start = ((frame_face.shape[0] - just_face.shape[0]) // 2, (frame_face.shape[1] - just_face.shape[1]) // 2)
    ...
    def show_frame(self, output_frame, iteration):
        if output_frame.shape[0] > 0 and output_frame.shape[1] > 0:
            if self.frame_substraction and iteration % 10 == 0 and self.active_mode != self.PHOTO_MODE:
                ...
                if iteration % 20 == 0:
                    if self.prev_frame1 is not None:
                        ...
                        # frame_diff_uint8 = apply_filter(frame_diff_uint8, frame_diff_uint8,
                        #                              selected_filter_option=self.filter_option)
                        #cv2.imshow('Detected Faces', abs(frame - self.prev_frame1))
                        cv2.imshow('Detected Faces', frame_diff_uint8)
                    ...
                    if self.prev_frame2 is not None:
                        ...
                        # Convert the difference back to uint8 format
                        frame_diff_uint8 = np.clip(frame_diff, 0, 255).astype(np.uint8)
                        #cv2.imshow('Detected Faces', abs(frame - self.prev_frame2))
                        cv2.imshow('Detected Faces', frame_diff_uint8)
                    self.prev_frame2 = frame
            # elif not self.frame_substraction:
            #     frame, _, _ = resize_frame(output_frame, 384)
